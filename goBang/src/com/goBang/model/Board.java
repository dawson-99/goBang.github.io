...
public class Board extends JPanel implements checkBoardService.size {
    ...
    public void paint(Graphics gs){
        ...
        gs.fillOval(x+(row-1)/2*size,y+(column-1)/2*size,5,5);//天元
        int tar=size*8;
        gs.fillOval(x+size*3,y+size*3,3,3);
        gs.fillOval(x+size*3+tar,y+size*3,3,3);
        gs.fillOval(x+size*3+tar,y+size*3+tar,3,3);
        gs.fillOval(x+size*3,y+size*3+tar,3,3);//四星
    }
}
